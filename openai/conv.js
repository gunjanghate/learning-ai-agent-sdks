...
async function main(q = ''){
    ...
    const res = await run(sqlAgent, q, sharedHistory);
    console.log(res.finalOutput.sqlQuery);

    sharedHistory.push({ role: 'agent', content: res.finalOutput.sqlQuery || 'No SQL Query Generated'})
}
...
// Turn 1
main("Hi, My name is GG").then(()=>{
    // Turn 2
    main('Get me all users with my name'); // here agent dont now the name so it should not be able to generate sql query
}) 